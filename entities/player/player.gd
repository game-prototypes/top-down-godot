...
func _shoot():
	...
	var target=get_local_mouse_position() # Get mouse position, using the current position as reference
	gun_raycast.cast_to=target # Where to ray cast?
	# Raycast, by default, will run on the next physics_process, because we want to run it now, we will force
	# the update
	...
	bullet_instance.set_path(position, target) # Draws the bullet trail path (check bullet_trail.tscn)
